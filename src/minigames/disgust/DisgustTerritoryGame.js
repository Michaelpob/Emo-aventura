...
export class DisgustTerritoryGame extends MinigameBase {
  ...
  decorateZone(zone) {
    ...
    if (kind === 'pantano') {
      ...
      for (let i = 0; i < 9; i += 1) {
        ...
        this.scene.add(rock);
        this.controller.addCollider({ type: 'sphere', center: new THREE.Vector3(rx, this.heightAt(rx, rz), rz), radius: 0.9 });
      }
      const back = makeRock({ color: '#2f3a33', scale: 4.2 });
      back.position.set(x + 2.5, this.heightAt(x + 2.5, z - 2.5) - 0.6, z - 2.5);
      this.scene.add(back);
      this.controller.addCollider({ type: 'sphere', center: new THREE.Vector3(x + 2.5, y, z - 2.5), radius: 2.2 });
    } else if (kind === 'bosque') {
      for (let i = 0; i < 9; i += 1) {
        ...
        this.scene.add(tree);
        this.controller.addCollider({ type: 'sphere', center: new THREE.Vector3(tx, this.heightAt(tx, tz), tz), radius: 0.55 });
      }
  ...
  buildPaths() {
    PATH_ORDER.forEach((id) => {
      ...
      const path = {
        ...
        dome: this.makeDome(def.x, def.z, 9.5, 0.5),
  ...
  buildArena() {
    ...
    this.altars = PATH_ORDER.filter((id) => id !== 'atencion').map((id, i, arr) => {
      ...
      icon.visible = false;
      this.scene.add(icon);
      const lb = makeText(`${def.technique}\n(aprende esta herramienta en su camino)`, { size: 0.9, maxChars: 24, color: '#cfd8c4' });
  ...
  buildHud() {
    if (this.root.querySelector('[data-dg-task]')) return;
  ...
  activateStimulus(zone) {
    ...
    if (!zone.entered) {
      zone.entered = true;
      this.say(zone.name.toUpperCase(), 2000);
    }
    ...
    this.dg.askHint.innerHTML = 'Si te genera desagrado, <b>aléjate</b>. Si no, deja que se acerque.';
  ...
  openPath(id) {
    ...
    path.closedSign.visible = false;
    this.feedback.tweenValue(path.dome.material, 'opacity', 0, 2.2);
    this.later(() => { path.dome.visible = false; }, 2300);
    this.audio.play('light', { volume: 0.35 });
  }
  ...
  bossTask() {
    const b = this.boss;
    return `👾 La Reacción Impulsiva · tamaño ${Math.max(0, b.size)} · cada altar la encoge 1`;
  }
  ...
  updateBoss(dt) {
    ...
    this.altars.forEach((al) => {
      if (al.cooldown > 0) {
        al.cooldown -= dt;
        if (al.cooldown <= 0 && this.learned.has(al.id) && !al.busy) { al.interact.enabled = true; al.stone.material.emissiveIntensity = 0.6; }
      }
    });
  ...
  useAltar(altar) {
    ...
    if (altar.id === 'respira') {
      ...
      this.say('COMPLETA 2 RESPIRACIONES EN LA BURBUJA', 2600);
      ...
      let found = 0;
      this.say('MIRA 3 ELEMENTOS DEL ENTORNO', 2600);
      b.task.update = (dt) => {
        this.camera.getWorldDirection(_dir);
        let best = 0;
      ...
      this.say('SOSTÉN LA SENSACIÓN Y RECONÓCELA', 2600);
      ...
      this.say('ATRAVIESA LA PUERTA EQUILIBRADA', 2600);
      ...
      this.audio.playAt('pad', g, { volume: 0.8, refDistance: 6 });
      this.say('ACÉRCATE AL GUARDIÁN DE CONFIANZA', 2600);
      b.task.update = () => {
        g.lookAt(this.controller.position.x, g.position.y, this.controller.position.z);
  ...
  altarDone(altar) {
    ...
    else this.say('LA CRIATURA SE ENCOGE', 1800);
  }
  ...
  victory() {
    ...
    this.setTask('');
    this.say('LA NIEBLA DESAPARECE', 3000);
    this.later(() => {
  ...
  onReset() {
    ...
    this.bars.clear();
    this.root.querySelector('.dg-hud')?.remove();
