...
| La Ciénaga Turbia | Desagrado (rechazo social y moral) | cámara fija | **rechazar la conducta, no a la persona** | Menú de la isla con dos minijuegos independientes, en cualquier orden. **La Cadena** (tiempo real, 5 rondas): eres el nodo central de una red sobre la ciénaga por la que corren mensajes; cortar el hilo con un arrastre, dejar pasar (se replica a 2-3 nodos y enturbia el agua), poner límite manteniendo pulsado tu nodo (eco con tres frases: asertiva, agresiva o evasiva) o llevar el paquete a la baliza para pedir ayuda. Un tercio son *trampas*: diferencias que dan grima pero no hacen daño; cortarlas penaliza. HUD de claridad del agua, coherencia con los valores declarados y termómetro de desagrado. La última ronda trae un mensaje sobre ti. **El Separador** (sin cronómetro): 10 grumos —escenas envueltas en una sustancia oscura— que se escuchan, se separan arrastrando (con resistencia), se clasifican en «Esto rechazo» (y se nombra la conducta), «Esta persona» o «Aquí no hay daño, hay diferencia», y cierran eligiendo un paso real que va al diario de límites. Los últimos son dilemas sin respuesta única. Cada juego termina con su reevaluación; la isla se cierra con el diario cuando están los dos |
...
│   ├── disgust/DisgustIslandFlow.js (menú) · CadenaGame.js · SeparadorGame.js · cienaga.js (escenario, voz)
